<script lang="ts">
  ...
  import { kids } from "../products"; // Importing products from external file
  ...
  const kidsId: any = $page.params.kidsId;

  // Adjust data retrieval based on men array
  const selectedProduct:any = kids.find(m => m.id === parseInt(kidsId));
  const qty = quantity[kidsId - 1];
...
<a href="/categories/kids" class="text-blue-600 underline" ><i class="mx-6 fa-solid fa-chevron-left "></i>Go back to Kids category</a>
