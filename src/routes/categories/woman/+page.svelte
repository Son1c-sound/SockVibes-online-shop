<script lang="ts">
  import { woman, socks, slippers } from "./products";
  ...
  $: if (api) {
    ...
    function navtoSlip(womanSlipperId:any) {
    ...
    function womanSock() {
      goto('/categories/woman/womanSocks')
    }
 
    function navtoWomanSock(womanSocksId: any) {
        goto(`/categories/woman/womanSocks/${womanSocksId}`);
    }  

    function womanSlipper(womanSlipperId: any) {
...
<div class="my-3 flex justify-center space-x-4 mx-auto">      
  <Select.Root portal={null}>
    ...
    <Select.Content>
      <Select.Group>
        <button on:click={womanSock}>woman socks</button>
        <hr>
        <button on:click={womanSlipper}>woman slippers</button>
...
>
  ...
  >
    ...
    <div class="p-2 sm:p-1 pt-0">
      <Button class="w-full bg-yellow-300 text-black hover:bg-yellow-400" on:click={() => navtoWomanSock(item.id)}>Purchase</Button>
    </div>
  </div>
{/each}
...
>
  ...
  <div class="p-2 sm:p-1 pt-0">
    <Button class="w-full bg-yellow-300 text-black hover:bg-yellow-400" on:click={() => navtoSlip(item.id)}>Purchase</Button>
  </div>
</div>
{/each}
</div>
