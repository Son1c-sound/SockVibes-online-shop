<script lang="ts">
  ...
  const womanSlipperId: any = $page.params.womanSlipperId;

  // Adjust data retrieval based on men array
  const selectedProduct:any = slippers.find(m => m.id === parseInt(womanSlipperId));
  const qty = quantity[womanSlipperId - 1];
...
<a href="/categories/woman/womanSocks" class="text-blue-600 underline" ><i class="mx-6 fa-solid fa-chevron-left "></i>Go back to Women category</a>
