<script lang="ts">
  ...
  import {underwears } from '../../products' // Importing products from external file
  ...
  const menUnderId: any = $page.params.menUnderId;

  // Adjust data retrieval based on men array
  const selectedProduct:any = underwears.find(m => m.id === parseInt(menUnderId));
  const qty = quantity[menUnderId - 1];
