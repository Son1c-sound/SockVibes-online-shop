<script lang="ts">
  ...
  import { newItems } from "../products"; // Importing products from external file
  ...
  const newitemsId: any = $page.params.newitemsId;

  // Adjust data retrieval based on men array
  const selectedProduct:any = newItems.find(m => m.id === parseInt(newitemsId));
  const qty = quantity[newitemsId - 1];
...
<a href="/categories/newitems" class="text-blue-600 underline" ><i class="mx-6 fa-solid fa-chevron-left "></i>Go back to new items</a>
